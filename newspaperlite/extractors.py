...
from .newspaperutils import StringReplacement, StringSplitter
...
class ContentExtractor(object):
	def __init__(self, remove_detected_blocks,  language='it'):
		self._tree_explorer = TreeExplorer
		self.language = language
		self.remove_detected_blocks = remove_detected_blocks
	...
	def update_language(self, meta_lang):
		...
		if meta_lang:
			...
			if tmp_class:
				...
				self.stopwords_class = tmp_class # da modificare in modo che possa lavorare anche con lingue come il cinese
				result = True
		return result
	...
	def get_authors(self, doc):
		...
		for attr in ATTRS:
			for val in VALS:
				...
				found = self._tree_explorer.get_elements_by_tag_name(doc, attr=attr, value=val)
	...
	def get_publishing_date(self, url, doc):
		...
		for known_meta_tag in PUBLISH_DATE_TAGS:
			meta_tags = self._tree_explorer.get_elements_by_tag_name(doc, attr=known_meta_tag['attribute'], value=known_meta_tag['value'])
			for m_tag in meta_tags:
				date_str = self._tree_explorer.get_attribute(m_tag, known_meta_tag['content'])
				datetime_obj = parse_date_str(date_str)
				if datetime_obj:
					return datetime_obj
				else:
					txt = self._tree_explorer.get_text(m_tag)
					datetime_obj = utils.get_date_from_string_by_language(self.language, txt)
					if datetime_obj:
						return datetime_obj

		body_node = self._tree_explorer.get_elements_by_tags(doc, ['body'])[0]
		html = self._tree_explorer.get_text(body_node)
		date_match = re.search(STRICT_DATE_REGEX, html)
		if date_match:
			date_str = date_match.group(0)
			datetime_obj = parse_date_str(date_str)
			if datetime_obj:
				return datetime_obj
		return utils.get_date_from_string_by_language(html, self.language)
	...
	def get_title(self, doc):
		...
		"""
		title = ''
		title_element = self._tree_explorer.get_elements_by_tag_name(doc, tag='title')
		# no title found
		if title_element is None or len(title_element) == 0:
			return title

		# title elem found
		title_text = self._tree_explorer.get_text(title_element[0])
		...
		# - clean double spaces
		title_text_h1 = ''
		title_element_h1_list = self._tree_explorer.get_elements_by_tag_name(doc, tag='h1') or []
		title_text_h1_list = [self._tree_explorer.get_text(tag) for tag in title_element_h1_list]
	...
	def get_feed_urls(self, source_url, categories):
		...
		for category in categories:
			...
			feed_elements = self._tree_explorer.get_elements_by_tags(category.doc, **kwargs)
	...
	def get_favicon(self, doc):
		...
		meta = self._tree_explorer.get_elements_by_tag_name(doc, **kwargs)
		if meta:
			favicon = self._tree_explorer.get_attribute(meta[0], 'href')
			return favicon
		return ''

	def get_meta_lang(self, doc):
		#return utils.detect_language_from_text()
		"""Extract content language from meta
		"""
		result = None
		attr = None
		og_md_attr = self._tree_explorer.xpath(doc, '//meta[@itemprop="inLanguage"] | //meta[@property="og:locale"]')
		if og_md_attr:
			attr = self._tree_explorer.get_attribute(og_md_attr[0], attr='content')
		# we have a lang attribute in html
		if attr is None:
			attr = self._tree_explorer.get_attribute(doc, attr='lang')
			if attr is None:
				...
				for item in items:
					meta = self._tree_explorer.get_elements_by_tag_name(doc, **item)
					if meta:
						attr = self._tree_explorer.get_attribute(meta[0], attr='content')
						break
		if attr:
			value = attr[:2]
	...
	def get_meta_content(self, doc, metaname):
		...
		"""
		meta = self._tree_explorer.css_select(doc, metaname)
		content = None
		if meta is not None and len(meta) > 0:
			content = self._tree_explorer.get_attribute(meta[0], 'content')
		if content:
	...
	def get_meta_img_url(self, article_url, doc):
		...
		if not try_one:
			...
			elems = self._tree_explorer.get_elements_by_tag_name(doc, use_regex=True, **link_img_src_kwargs)
			...
			if not try_two:
				...
				if not try_three:
					...
					elems = self._tree_explorer.get_elements_by_tag_name(doc, **link_icon_kwargs)
	...
	def get_meta_data(self, doc):
		...
		properties = self._tree_explorer.css_select(doc, 'meta')
	...
	def get_canonical_link(self, article_url, doc):
		...
		2. The og:url tag
		"""
		links = self._tree_explorer.get_elements_by_tag_name(doc, tag='link', attr='rel',
											 value='canonical')

		canonical = self._tree_explorer.get_attribute(links[0], 'href') if links else ''
	...
	def get_img_urls(self, article_url, doc):
		...
		"""
		img_kwargs = {'tag': 'img'}
		img_tags = self._tree_explorer.get_elements_by_tag_name(doc, **img_kwargs)
	...
	def get_first_img_url(self, article_url, top_node):
		...
		"""
		node_images = self._tree_explorer.get_elements_by_tag_name(node=top_node, tag='img', attr='src')
		# node_images = self.get_img_urls(article_url, top_node)
		# node_images = list(node_images)
		if len(node_images):
			return node_images[0].attrib['src']
	...
	def _get_urls(self, doc, titles):
		...
		a_kwargs = {'tag': 'a'}
		a_tags = self._tree_explorer.get_elements_by_tag_name(doc, **a_kwargs)
	...
	def get_urls(self, doc_or_html, titles=False, regex=False):
		...
		if isinstance(doc_or_html, str):
			doc = self._tree_explorer.fromstring(doc_or_html)
		else:
			doc = doc_or_html
	...
	def extract_tags(self, doc):
		if len(list(doc)) == 0:
			return NO_STRINGS
		elements = self._tree_explorer.css_select(doc, A_REL_TAG_SELECTOR)
		if not elements:
			elements = self._tree_explorer.css_select(
		...
		tags = []
		for el in elements:
			tag = self._tree_explorer.get_text(el)
			if tag:
				tags.append(tag)
		return set(tags)
	...
	def calculate_best_node(self, doc):
		top_node = None
		tmp = self._tree_explorer.xpath(doc, '//*[(contains(@itemprop, "articleBody"))]'
		...
		if tmp:
			...
			tmp = self._tree_explorer.xpath(doc, '//article[@itemtype="http://schema.org/Article"]'
												'|//article[@itemtype="http://schema.org/NewsArticle"]')
			if tmp:
				doc = tmp[0]
			else:
				tmp = self._tree_explorer.xpath(doc, '//article[contains(@id,"main")]'
													 '|//article[contains(@id, "artic")]'
													  '//article[contains(@class, "artic")]'
													 '|//div[contains(@class, "bodyNews")]'
													 '|//div[contains(@id, "bodyNews")]'
													 '|//div[contains(@class, "bodynews")]'
													 '|//div[contains(@id, "bodynews")]'
													 '|//div[contains(@class, "body-news")]'
													 '|//div[contains(@id, "body-news")]'
													 '|//div[contains(@class, "body_news")]'
													 '|//div[contains(@id, "body_news")]'
													 '|//div[contains(@id, "content-artic")]'
													 '|//div[contains(@class, "content-artic")]'
													 '|//div[contains(@id, "article-content")]'
													 '|//div[contains(@class, "article-content")]')
				if tmp:
					for t in tmp:
						style = self._tree_explorer.get_attribute(t, attr='style')
						if style and style.find('display') >= 0:
							index = style.find('display')
							index_separator = style[index:].find(';')
							if index_separator >= 0:
								style = style[index:index_separator]
							else:
								style = style[index:]
		...
		for node in nodes_to_check:
			#text_node = self._tree_explorer.get_text(node)
# 			word_stats = self.stopwords_class.get_stopword_count(text_node)
# 			high_link_density = self.is_highlink_density(node)
# #			is_visible = self.is_visible_node(node)
# 			if word_stats.get_stopword_count() > 2 and not high_link_density:
 			nodes_with_text.append(node)

		nodes_number = len(nodes_with_text)
		if nodes_number == 0:
			text_node = self._tree_explorer.get_text_without_child(doc)
			word_stats = self.stopwords_class.get_stopword_count(text_node)
			high_link_density = self.is_highlink_density(doc)
			if word_stats.get_stopword_count() > 2 and not high_link_density:
				nodes_with_text.append(doc)
				nodes_number = 1
		negative_scoring = 0
		...
		for node in nodes_with_text:
			...
			parent_node = self._tree_explorer.get_parent(node)
			...
			parent_parent_node = self._tree_explorer.get_parent(parent_node)
		...
		if top_node is not None:
			result = copy.deepcopy(top_node)
			if self.remove_detected_blocks:
				self._tree_explorer.remove(top_node)
		return result
	...
	def is_boostable(self, node):
		...
		for current_node in nodes:
			# <p>
			current_node_tag = self._tree_explorer.get_tag(current_node)
			if current_node_tag == para:
				if steps_away >= max_stepsaway_from_node:
					return False
				paragraph_text = self._tree_explorer.get_text(current_node)
	...
	def walk_siblings(self, node):
		return self._tree_explorer.previous_siblings(node)
	...
	def get_siblings_content(
		...
		"""
		if current_sibling.tag == 'p' and len(self._tree_explorer.get_text(current_sibling)) > 0:
			e0 = current_sibling
			if e0.tail:
				e0 = copy.deepcopy(e0)
				e0.tail = ''
			...
			potential_paragraphs = self._tree_explorer.get_elements_by_tag_name(current_sibling, tag='p')
			if potential_paragraphs is None:
				return None
			else:
				ps = []
				for first_paragraph in potential_paragraphs:
					text = self._tree_explorer.get_text(first_paragraph)
					if len(text) > 0:
						word_stats = self.stopwords_class.get_stopword_count(text)
						...
						if score < paragraph_score and not high_link_density:
							p = self._tree_explorer.create_element(
	...
	def get_siblings_score(self, top_node):
		...
		paragraphs_number = 0
		paragraphs_score = 0
		nodes_to_check = self._tree_explorer.get_elements_by_tag_name(top_node, tag='p')
	...
	def update_score(self, node, add_to_score):
		...
		"""
		current_score = 0
		score_string = self._tree_explorer.get_attribute(node, 'gravityScore')
		if score_string:
			current_score = float(score_string)

		new_score = current_score + add_to_score
		self._tree_explorer.set_attribute(node, "gravityScore", str(new_score))

	def update_node_count(self, node, add_to_count):
		"""Stores how many decent nodes are under a parent node
		"""
		current_score = 0
		count_string = self._tree_explorer.get_attribute(node, 'gravityNodes')
		if count_string:
			current_score = int(count_string)

		new_score = current_score + add_to_count
		self._tree_explorer.set_attribute(node, "gravityNodes", str(new_score))
	...
	def is_highlink_density(self, e):
		...
		"""
		links = self._tree_explorer.get_elements_by_tag_name(e, tag='a')
		if not links:
			return False

		text = self._tree_explorer.get_text(e)
		...
		sb = []
		for link in links:
			sb.append(self._tree_explorer.get_text(link))
	...
	def get_node_gravity_score(self, node):
		gravity_score = self._tree_explorer.get_attribute(node, 'gravityScore')
		if not gravity_score:
			return None
		return float(gravity_score)

	def check_node_text(self, node):
		result = False
		text_node = self._tree_explorer.get_text(node)
		word_stats = self.stopwords_class.get_stopword_count(text_node)
		high_link_density = self.is_highlink_density(node)
	...
	def nodes_to_check(self, doc):
		...
		nodes_to_check = []
		for tag in ['p', 'pre', 'td']:
			items = self._tree_explorer.get_elements_by_tag_name(doc, tag=tag)
			if items:
				for item in items:
					if self.check_node_text(item):
				...
				if items:
					for item in items:
						if self.check_node_text(item):
							nodes_to_check.append(item)
		if len(nodes_to_check) == 0:
			items = self._tree_explorer.xpath(doc, '//*[contains(@itemprop, "text")]')
			if items:
				for item in items:
					if self.check_node_text(item):
						nodes_to_check.append(item)
		if len(nodes_to_check) == 0:
			items = self._tree_explorer.get_elements_by_tag_name(doc, tag='div')
			if items:
				for item in items:
					if self.check_node_text(item):
						nodes_to_check.append(item)
	...
	def is_table_and_no_para_exist(self, e):
		sub_paragraphs = self._tree_explorer.get_elements_by_tag_name(e, tag='p')
		for p in sub_paragraphs:
			txt = self._tree_explorer.get_text(p)
			if len(txt) < 25:
				self._tree_explorer.remove(p)

		sub_paragraphs_2 = self._tree_explorer.get_elements_by_tag_name(e, tag='p')
	...
	def post_cleanup(self, top_node):
		...
		for e in self._tree_explorer.get_children(node):
			e_tag = self._tree_explorer.get_tag(e)
			if e_tag != 'p':
				if self.is_highlink_density(e):
					self._tree_explorer.remove(e)
		return node

	def get_sections(self, doc):
		result = list()
		tmp = self._tree_explorer.xpath(doc, '//meta[@itemprop="articleSection"]')
		if tmp:
			result.append(self._tree_explorer.get_attribute(tmp[0], 'content'))
		if result is None:
			tmp = self._tree_explorer.xpath(doc, '//meta[@name="sections"]')
			if tmp:
				result.append(self._tree_explorer.get_attribute(tmp[0], 'content'))
		if result:
			return result
